...
const VideoCarousel: React.FC<VideoCarouselProps> = ({ className = '' }) => {
  ...
  // Optimized preload strategy: only preload next video when current is playing
  useEffect(() => {
    // Set preload for current video to 'auto' (needed immediately)
    const currentVideo = videoRefs.current[currentIndex];
    if (currentVideo) {
      currentVideo.preload = 'auto';
    }

    // Preload next video with metadata only (lighter load)
    ...
    // Set all other videos to 'none' to save bandwidth
  ...
  useEffect(() => {
    videoRefs.current.forEach((video, index) => {
      ...
      const shouldPlay =
        ...
        (isTransitioning &&
          (index === currentIndex || index === prevIndexRef.current));
